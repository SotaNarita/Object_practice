...
public class RandomGenerate {
	...
	public static void carGenerate() {
		...
		for(int i=0; i<l; i++) {
			...
			priceSum += hondaArray[i].getPrice();
			System.out.println(hondaArray[i].getPrice());
		}
		for(int j=0; j<m; j++) {
			nissanArray[j] = new Nissan();
			priceSum += nissanArray[j].getPrice();
			System.out.println(nissanArray[j].getPrice());
		}
		for(int k=0; k<n; k++) {
			ferrariArray[k] = new Ferrari();
			priceSum += ferrariArray[k].getPrice();
			System.out.println(ferrariArray[k].getPrice());
		}
